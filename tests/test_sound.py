...
from outils.widgets.device_card import DeviceCard, card_id
...
def card(app: OutilsApp, device: Device) -> DeviceCard:
    return app.query_one(f"#{card_id(device)}", DeviceCard)
...
class SoundTest(unittest.TestCase):
    ...
    def test_a_pactl_failure_while_reloading_is_said_once_in_the_header(self):
        async def body(app, pilot, mocks):
            with patch("outils.pactl.mixer", side_effect=PactlError("Connection failure: Connection refused")):
                ...
                await settle(app, pilot)
                self.assertEqual(footer_message(app), "Connection failure: Connection refused")
    ...
    def test_opens_on_the_output_in_use_and_no_microphone_hides_its_section(self):
        async def body(app, pilot, mocks):
            ...
            self.assertEqual(len(app.query(DeviceCard)), 2)
            self.assertFalse(app.query_one("#inputs-cards").display)

        self.run_app(body, mixer=Mixer(outputs=[replace(LAPTOP, default=False), replace(MONITOR, default=True)]))
    ...
    def test_arrows_change_the_volume_and_m_mutes_then_the_bar_is_told(self):
        async def body(app, pilot, mocks):
            ...
            # Down goes on into the microphones
            await pilot.press("down", "m")
            await settle(app, pilot)
            mocks["set_mute"].assert_called_with(MIC, True)
            self.assertEqual(mocks["refresh_bar"].call_count, 6)
    ...
    def test_clicking_disconnected_headphones_connects_and_switches_to_them(self):
        async def body(app, pilot, mocks):
            ...
            # Its Connect button stays in the Bluetooth column, right of the other cards' Mute
            mute = card(app, LAPTOP).query_one(".btn-mute").region
            self.assertEqual(offline.query_one(".btn-bluetooth").region.x, mute.right + 1)
    ...
    def test_enter_uses_a_device_and_reports_errors(self):
        async def body(app, pilot, mocks):
            ...
            self.assertEqual(footer_message(app), "Failure: No such entity")
